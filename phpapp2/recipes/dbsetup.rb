execute "mysql-create-table" do
  command "/usr/bin/mysql -u#{node[:deploy][:myphpapp2][:database][:username]} -p#{node[:deploy][:myphpapp2][:database][:password]} #{node[:deploy][:myphpapp2][:database][:database]} -e'CREATE TABLE chat(
